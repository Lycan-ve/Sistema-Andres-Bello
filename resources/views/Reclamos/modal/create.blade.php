<div class="modal fade" id="reclamo" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    ...
    <form method="post" class="p-4 md:p-5">
        @csrf
        <div class="flex justify-center">Libro a Seleccionar</div>
        <hr class="mb-3" title=""/>
                ...
                <div class="col-span-2">
                    ...
                    <select name="id_libros" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">
                        ...
                        @foreach($libros as $libro)
                            <option value="{{$libro->id}}">{{$libro->titulo}} - {{$libro->ano_academico->nombre}}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-span-2">
                        <label for="name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Cantidad</label>
                        <input type="number" name="cantidad" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Ingrese la Cantidad" required="">
                    </div>
                </div>
                <div class="flex justify-center">Datos del Reclamante</div>
                <hr class="mb-3" title=""/>
                <div class="grid gap-4 mb-4 grid-cols-2">
                    <div class="col-span-2">
